"""Guard against broken internal `.md` links and anchors.
...
heading text** — so renaming a section silently breaks every incoming reference. The
deterministic "ensure no link dangles" step is an instance of "scripts over instructions": an
exit code, not a reviewer checklist item.
...
will not match GitHub's. The parser is a direct development dependency, so there is no added
cost for it.
...
ones: a reference to an existing file that ceased to be a topic's home is indistinguishable from
a correct one to the guard. This is the same "presence != correctness" as in
`test_doc_headers.py` and `test_adr_records.py`; a human catches the discrepancy in review. The
second boundary is **form**:
...
_EXPECTED_SCOPE_DIRS = ("docs/architecture", "docs/adr")
...
class TestLinkPredicates:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("Secret scan (`secrets`)", "secret-scan-secrets"),
        ],
    )
