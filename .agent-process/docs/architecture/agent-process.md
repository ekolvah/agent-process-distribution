...
| --- | --- | --- | --- |
| `planner` | Tracking issue, repository context, and user decisions | A validated OpenSpec change (`openspec/changes/<name>/`) with its architect review | human approval, then `implementer` |
...
architect review → tasks. What this project adds to every artifact is the
...
an unmapped scenario is a finding) and the `tasks` rule, which is the
delivery flow below as tasks of the change. `openspec/specs/` is what the
