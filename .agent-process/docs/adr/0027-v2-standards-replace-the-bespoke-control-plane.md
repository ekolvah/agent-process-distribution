...
Observations from v2-1 (`v2-1-planning-workflow`, #111), answering the seven open questions
...
* Review budget: the three-round cap lives in the `tasks` rule as prose; the round count of
  this PR is in its body. A counter in `wait_for_pr` waits for an observed overrun.
