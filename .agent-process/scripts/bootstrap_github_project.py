...
process-owned ``Planned`` option to an existing Project requires a separate
...
def main(argv: list[str] | None = None) -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="allow appending Planned to built-in Status in existing mode",
    )
    ns = parser.parse_args(argv)
    if ns.confirm_status_setup and MODE != "existing":
        parser.error("--confirm-status-setup is only valid in existing mode")
    if _already_configured() and not ns.confirm_status_setup:
