#!/usr/bin/env python3
"""Move an issue's Status field in the configured GitHub Project.

Usage: python .agent-process/scripts/set_issue_status.py <N> <planned|in-progress>  (N = bare issue number)
...
`Todo` and `Done` are rejected: the built-in Project workflows own them, and a
second writer would only race them.
...
This module loads the tracked ``project_settings.py`` configuration, as does
``set_issue_priority.py``, so both scripts operate on the same Project.
...
non-zero exit. Any nonzero `gh` exit is surfaced rather than being reported as
a set status. `gh` is the sole external boundary, run through a single `_run`
seam so tests mock `subprocess.run`.
"""
...
try:
    ...
    from .project_settings import (
        STATUS_OPTION_IDS as OPTION_IDS,
    )
...
def option_id_for_status(status: str) -> str:
    ...
    Unknown or process-external status → ValueError (visible), never a silent default."""
...
def _checked(cmd: list[str], what: str) -> str:
    """Run `cmd`; on non-zero exit raise with the captured stderr (visible failure)."""
    result = _run(cmd)
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Move an issue's Status in the tracked GitHub Project."
    )
