...
# Extension point: add one `capture_<system>` function plus a matching subparser
# per external, read-only system this project's discovery/evidence work needs
# (each following the `stdin`/`github` shape above — a `--confirm-repository-safe`
# flag and no argument that lets the caller construct an arbitrary side-effecting
# command). Keep every route read-only; a route that can mutate the external
# system does not belong in this script.
