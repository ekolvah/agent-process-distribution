...
is removed with its cause: `.agent-process/scripts/check_agent_review_outcome.py` no longer knows paths,
`.agent-process/scripts/review_gate.py` no longer knows controller classification, and the manual IDE-review policy in
