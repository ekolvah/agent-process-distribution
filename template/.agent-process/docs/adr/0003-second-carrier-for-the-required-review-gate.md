...
* **The second carrier must use the owner’s subscription.** There is no separate paid API key; a carrier
  ...
  GitHub integration.
* **The validity rule must not gain a second home.** `.agent-process/scripts/check_agent_review_outcome.py` already knows
...
`.agent-process/scripts/request_codex_review.py`: it reads existing reviews, posts `@codex review` once when the current
