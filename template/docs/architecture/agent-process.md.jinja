...
1. Run `python scripts/validate_issue_sections.py <N> --mark-planned`. A
...
3. The implementer validates the issue again, verifies its Project Priority
   ...
   with `python scripts/issue_branch.py <N>` (also moves the board card to
...
`python -m scripts.review_gate <PR>` reads the live PR — required contexts on
...
split, run `python scripts/check_consumer_test_collision.py <path>`: it
renders the current template and reports, with the exact colliding relative
path, any file already occupying a location the template's
`tests/agent_process/` subtree reserves for itself — a case Copier's own
`--conflict` handling does not catch, since it only marks conflicts for paths
it previously tracked through a prior render's diff.
...
5. The process owns exactly two board Status transitions, written from
   ...
   built-in Project automations; `scripts/set_issue_status.py` rejects them.
...
`python scripts/agent_orchestrator.py <state.json>` is read-only advice, never
