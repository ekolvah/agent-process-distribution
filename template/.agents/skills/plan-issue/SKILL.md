...
- There is no local discovery subagent here either, so run the observation
  yourself against
  [the discovery runbook](../../../.agent-process/docs/architecture/agent-process.md#discovery-runbook),
  ...
  `python .agent-process/scripts/validate_issue_sections.py <N> --evidence-only --body-file <file>`
  before recording it in the body.
- There is no local reviewer subagent here, so perform the architect review
  yourself against
  [the architect review contract](../../../.agent-process/docs/architecture/agent-process.md#architect-review-contract),
  reading it and `.agent-process/docs/architecture/principles.md` rather than working from
