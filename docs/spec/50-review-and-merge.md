...
- **REVW-2** (MUST) The reviewer reads the **diff** of the PR, not whole files, together
  with the issue's acceptance criteria, and reports a test that does not actually check
  the criterion it is mapped to (PLAN-8). The PR body lists tracked deferrals as issue
  links so a reviewer does not re-report them.
- **REVW-3** (MUST) A fix after the implementing run has ended (IMPL-5) is the person
  launching `/implement #N` again; that run reads the open threads.
...
- **REVW-9** (MUST) A PR is mergeable only when every acceptance criterion of its issue
  is covered by a test that ran green in the PR's checks, or is marked `n/a: <reason>`
  in the plan the person approved (PLAN-8). An uncovered criterion is a failed check,
