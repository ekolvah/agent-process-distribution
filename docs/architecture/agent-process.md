...
`reusable-agent-review.yml`. The PR author starts the sole carrier with
`@codex review`; the workflow never posts that command and never invokes a
fallback model. It waits for Codex's normal GitHub review on the current head,
then translates its P0/P1/P2/P3 inline comments into the gate outcome. A valid
Codex `clean`, `rework`, or `blocking` verdict is final for that head.
