...
1. Read the `## Architect review` section of `skills/agent-process/SKILL.md` and
   `skills/agent-process/architect-review.schema.json` — the path is the one your other steps
   use, from the repository root: it is your contract and the structure of the file you
   write. Read repository-specific context from `openspec/config.yaml`.
