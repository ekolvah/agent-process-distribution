...
- [ ] 4.1 `git status --short` empty for this change's files → `python .agent-process/scripts/archive_change.py v2-2b-loop-script` (marks its own task, archives — applying the delta to `openspec/specs/` —, commits, pushes)
