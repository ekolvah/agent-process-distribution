...
under `_PROCESS_PATHS`, and, when `_has_product_scope()` and the root `pyproject.toml` holds the
single-rule setup of D2, `python -m pylint --rcfile pyproject.toml` on the remaining files. An
empty product list skips that run, as `check_mypy` does. A product scope without that setup
prints that it is not checked: any other pylint config runs rules unrelated to module size, and
a consumer would get limits it did not configure (D6). Any non-zero pylint exit (a bit mask; 16 = convention) fails through
