...
- [x] 4.1 With a clean worktree, run `python skills/agent-process/scripts/archive_change.py release-drift-check`. Verify that it applies the `distribution` delta, commits the archive, and pushes the branch
...
- `distribution / Release drift` → `tests/publisher/test_release_drift.py::test_release_drift`
- `distribution / Publisher checkout` → `tests/publisher/test_release_drift.py::test_publisher_checkout_is_exempt`
