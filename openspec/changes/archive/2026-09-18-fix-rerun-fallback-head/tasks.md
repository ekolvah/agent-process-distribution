...
- review-and-merge / Re-run on an interrupted fallback → `tests/publisher/test_request_codex_review.py::test_wait_is_present_for_either_trusted_reviewer_on_the_head` (review nodes under `github-actions` on the head and no closing comment → absence, exit 3) and `tests/publisher/test_reusable_workflows.py::test_agent_review_waits_for_codex_falls_back_to_claude_and_enforces_threads` (the prompt makes the closing comment the last write of every review)
