...
- **Specs and planning = OpenSpec.** Living spec in `openspec/specs/`, changes as deltas
  ...
  `openspec archive` inside the implementing PR before merge; project rules via `openspec/config.yaml`, architect review as
...
Each change v2-N is one PR against `main`, archived inside the PR before merge (tracking issue #107); v1
