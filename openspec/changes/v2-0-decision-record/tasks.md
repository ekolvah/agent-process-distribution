...
- [ ] 1.1 Create `.agent-process/docs/adr/0027-v2-standards-replace-the-bespoke-control-plane.md`
...
- [ ] 1.2 `Context and Problem Statement`: from `design.md` §Context and `proposal.md` §Why — the
...
- [ ] 1.3 `Decision Drivers`: the goal function in strict order (future support, tokens,
...
- [ ] 1.4 `Considered Options` as a Kepner-Tregoe trade study for the spec/plan carrier:
...
- [ ] 1.5 `Decision Outcome`: the eight decisions of `design.md` §Decisions as one bullet each,
  ...
  worse than v1).
- [ ] 1.6 `Native alternatives considered`: for every script v2 keeps or adds (`check_red`,
...
- [ ] 1.7 `Deletion condition`: what is removed if v2 stops paying for itself — per decision, one
...
- [ ] 1.8 `More Information`: superseded records listed with one line each on what replaces them;
...
- [ ] 2.1 Set `status: "superseded by ADR-0027"` in 0011, 0015, 0019, 0021, 0022, 0023 under
...
- [ ] 4.1 `python -m pytest tests/agent_process/test_adr_records.py tests/agent_process/test_doc_links.py -q` green
  (record name, unique number, known status, `superseded by` resolves, required sections; links).
- [ ] 4.2 `npx -y @fission-ai/openspec@latest validate v2-0-decision-record --strict` green (delta well-formed against the
  baseline spec).
- [ ] 4.3 `python .agent-process/scripts/ci_check.py` green.

## 5. Deliver (v1 path; `finish_change` arrives with v2-1)

- [ ] 5.1 Push the branch `v2-0-decision-record`; open the PR (`Closes #110`); address review threads.
