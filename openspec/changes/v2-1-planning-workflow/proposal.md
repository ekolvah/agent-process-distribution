...
- `implement-change` skill wraps `openspec-apply-change` with the GitHub steps: tracking
  issue, linked branch, `In progress`, `check_red`, `ci_check`, PR, `wait_for_pr`.
...
- Added: `openspec/schemas/agent-process/`, `openspec/config.yaml` rules, `skills/implement-change/`,
