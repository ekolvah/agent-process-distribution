...
- [ ] 5.1 Run `npx -y @fission-ai/openspec@1.13.0 validate --strict --all` and verify every change and spec passes
- [ ] 5.2 Run `python .agent-process/scripts/ci_check.py` and verify it passes, including `==> module-size`; confirm `git diff --name-only origin/main` lists only the paths of the proposal's Impact and the change directory

## 6. Deliver

- [ ] 6.1 With a clean worktree run `python skills/agent-process/scripts/archive_change.py code-complexity-limits`; verify it applies the `implementation` delta, commits the archive, and pushes the branch
