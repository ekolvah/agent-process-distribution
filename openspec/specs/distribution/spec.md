...
The quality job SHALL invoke the reusable workflow from this repository at the tag the
caller pins. The reusable workflow SHALL read the PR-to-issue link, run the caller's
declared `setup` and `test` commands against the PR worktree, and validate OpenSpec there;
the PR SHALL NOT replace the reusable workflow implementation selected by the pinned ref.

#### Scenario: Quality check on a PR
- **WHEN** the quality workflow runs for a PR
- **THEN** its process-owned steps come from the pinned reusable workflow and its caller-declared commands run against the PR head
