...
def test_quality_check_on_a_pr() -> None:
    ...
    assert call["inputs"] == {
        "setup": {"description": "Optional consumer setup command.", "required": False, "type": "string", "default": ""},
...
def test_one_caller_and_publisher_inputs() -> None:
    assert {p.name for p in WORKFLOWS.glob("*.yml")} == {"agent-process.yml", "reusable-quality.yml"}
...
def test_direct_advisory_reviews() -> None:
    ...
    for forbidden in ("request_codex_review", "check_blocking_review_threads", "review_gate", "fallback"):
...
def test_review_is_visible_but_not_required() -> None:
    ...
    ruleset = json.loads((ROOT / "skills" / "agent-process" / "templates" / "ruleset.json").read_text(encoding="utf-8"))
    required = next(r for r in ruleset["rules"] if r["type"] == "required_status_checks")
    assert [c["context"] for c in required["parameters"]["required_status_checks"]] == ["quality / quality"]


def test_dependabot_tracks_github_actions() -> None:
    config = yaml.safe_load((ROOT / ".github" / "dependabot.yml").read_text(encoding="utf-8"))
    entry = next(item for item in config["updates"] if item["package-ecosystem"] == "github-actions")
