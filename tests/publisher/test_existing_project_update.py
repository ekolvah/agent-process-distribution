...
def test_previous_release_update_preserves_consumer_owned_bytes(tmp_path: Path) -> None:
    ...
    install_payload(tmp_path, {".agent-process/ownership.yml": b"version: 1\n"})

    update_payload(tmp_path, {".agent-process/ownership.yml": b"version: 2\n"})

    assert product.read_bytes() == b"[project]\nname = 'product'\n"
    ownership = tmp_path / ".agent-process/ownership.yml"
    assert ownership.is_file()
    assert ownership.read_bytes() == b"version: 2\n"
...
def test_successful_update_is_idempotent(tmp_path: Path) -> None:
    payload = {".agent-process/ownership.yml": b"version: 2\n"}
    install_payload(tmp_path, {".agent-process/ownership.yml": b"version: 1\n"})

    update_payload(tmp_path, payload)
    ownership = tmp_path / ".agent-process/ownership.yml"
    assert ownership.is_file()
    first = ownership.read_bytes()
    update_payload(tmp_path, payload)

    assert (tmp_path / ".agent-process/ownership.yml").read_bytes() == first
